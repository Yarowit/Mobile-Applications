...
//    fun loadAllByIds(eventIds: IntArray): List<Event>
    ...
    @Query("SELECT * FROM Stats WHERE startTime >= :Start AND startTime < :Start + 86400000 AND probingInterval=86400000 AND (app = :name OR appType = :name)")
...
//    fun getDataByWeekAndName() : List<Stats>
    ...
    // do wykresu
    @Query("SELECT SUM(useTime) FROM Stats WHERE startTime >= :Start AND startTime < :Start + 86400000 AND probingInterval=86400000 AND appType = :type")
    fun getDailyUseTimeByDateAndType(Start: Long, type : String): Long

    // do kołowego
    @Query("SELECT * FROM Stats WHERE startTime >= :Start AND startTime < :Start + 86400000 AND probingInterval=86400000 AND appType = :type")
